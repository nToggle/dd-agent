...
class SDDockerBackend(AbstractSDBackend):
    ...
    def _extract_port_from_list(self, ports, tpl_var):
        ...
        idx = self._is_int(tpl_parts[-1])
        # an index was specified
        if idx is not False and idx < len(ports):
            return ports[idx]
        else:
            log.error("Invalid index in template variable '%s'. Trying "
                      "with the last element instead" % (tpl_var))
            return ports[-1]
